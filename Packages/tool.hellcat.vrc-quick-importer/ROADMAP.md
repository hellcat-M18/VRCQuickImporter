...
> 現状の進捗: ライブラリ一覧UIはBOOTHスキリスト風カードグリッドで実装済み。初期同期テストとして、ログイン済みWebView2 helperでBOOTHライブラリページを開き、DOMから商品候補を `database.json` に保存する導線を実装済み。実ファイル一覧取得・実ダウンロード・実インポートは未実装。
...
4. BOOTH ライブラリ 1 ページ分の手動同期（初期DOM抽出テストは実装済み。精度調整は運用テストで継続）
5. Unity EditorWindow の自作UIによる商品/ダウンロードファイルの一覧表示（カードグリッドUI実装済み。実ファイル一覧は未実装）
6. ダウンロード
