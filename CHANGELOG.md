...
### Fixed
- カード単位の「情報を再取得」を、注文履歴経由ではなく対象商品が掲載されているBOOTHライブラリ1ページだけを再取得する方式に戻し、`取得中…` から進まない問題を修正
- 長いファイル名が同じ省略表示になるとUnityのドロップダウンが項目を統合し4件目が選べない問題を、選択肢に連番と末尾保持の中間省略を付けて修正

## 0.2.5

### Added
- 初回セットアップ時に専用WebView2プロファイルでBOOTHの認証状態を確認
- 未認証時は可視WebViewでログインを促し、BOOTHライブラリへの到達後に初回同期を開始

### Fixed
- ログインキャンセル・タイムアウト・認証エラー時に待機し続けず、同期状態を復旧
- 通常同期中に認証切れが発生した場合も、待機し続けずエラーとして終了
- 認証後もBOOTHライブラリアクセス間隔5秒を守るよう修正

## 0.2.4

### Added
- 起動時バージョンチェック: GitHub Releasesの最新版と比較し更新通知バナーを表示

### Changed
- 増分同期をスロット単位の重複排除に変更（新商品追加時も既存商品が欠落しない）
- JSON保存をFile.Replaceによるアトミック操作に変更（途中停止時の破損防止）
- 通知システムをNIM_MODIFY使用に変更（連続通知の欠落防止）

### Fixed
- ダウンロード完了後にhelperプロセスが終了しないバグを修正（10秒安全弁 + SafeClose）
- ZIP展開のパストラバーサル脆弱性を修正
- 更新通知バナーの再描画対応・セッション内1回制限

### Security
- ZIP展開先のパス検証を追加（展開ディレクトリ外への書き込みを防止）
